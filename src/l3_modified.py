...
''' adding entropy calculation logic'''
import math
...
class Entropy(object):
	...
	def __init__(self):
		...
		self.destIPFrequency = {}
		self.entropy_value = 1

	def keeptrack(self, ip):

		self.current_window_size += 1
		#print(ip)

		if ip not in self.destIPFrequency:
			self.destIPFrequency[ip] = 1
		else:
			self.destIPFrequency[ip] += 1
		
		if self.current_window_size == MAX_WINDOW_SIZE:
	...
	def CalculateEntropy(self):
		...
		for ip, count in self.destIPFrequency.items():

			p = count/float(MAX_WINDOW_SIZE)
			# print(p)
			elist.append(-p * math.log(p, 10))

		self.entropy_value = sum(elist)
		#print("from entropy func",self.entropy_value)
		
		

	def ResetDataStructures(self):
		print("ds reset")
		self.destIPFrequency = {}
		self.current_window_size = 0

'''end of entropy calculation logic'''
...
class l3_switch (EventMixin):
  def __init__ (self, fakeways = [], arp_for_unknowns = False, wide = False):
    ...
    self.switch_Port_Packet_Frequency = {}
    self.entropy_obj = Entropy()
    self.one_time_timer_set = False
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    ''' add timer function for recoco Timer'''
    
    def _timer_function ():

      # if self.one_time_timer_set == True:
        #print datetime.datetime.now(),": calling timer fucntion now!!!!!" 
        for switch_id, port_packet_frequency in self.switch_Port_Packet_Frequency.items():
          for port_id, packet_frequncy in port_packet_frequency.items():
            if packet_frequncy >=50:
              print("-----------------------------------------------------------------------")
              print(datetime.datetime.now(),"*******-----DDOS DETECTED----********")
              print(datetime.datetime.now(),": BLOCKED PORT NUMBER  : ", str(port_id), " OF SWITCH ID: ", str(switch_id))
              print("-----------------------------------------------------------------------")
              msg = of.ofp_packet_out(in_port=port_id)
              core.openflow.sendToDPID(switch_id,msg)     
    ...
    if isinstance(packet.next, ipv4):
      ...
      '''' editing packet_in logic'''

      
      #print("isinstance")
      self.entropy_obj.keeptrack(packet.next.dstip)
      print("/////ENTROPY VALUE : ", self.entropy_obj.entropy_value, "//////")
	
      if self.entropy_obj.entropy_value < MIN_ENTROPY_THRESHOLD:
        #update dictionary
        # if dict empty
        if not bool(self.switch_Port_Packet_Frequency): #empty dictionary returns bool as false
          self.switch_Port_Packet_Frequency[dpid] = {}
          self.switch_Port_Packet_Frequency[dpid][inport] = 1
        else:
          # if switch dpid not in dict
          if dpid not in self.switch_Port_Packet_Frequency:
            self.switch_Port_Packet_Frequency[dpid] = {}
            self.switch_Port_Packet_Frequency[dpid][inport] = 1
          else:
            #if switch is there but port not there
            if inport not in self.switch_Port_Packet_Frequency[dpid]:
              self.switch_Port_Packet_Frequency[dpid][inport] = 1
        ...
        #call timer function
        if self.one_time_timer_set == False:
          Timer(2, _timer_function, recurring=True)
          self.one_time_timer_set = False
      else:
        self.one_time_timer_set = False
        #print("Check\n")
